...
CURLcode qcurl_easy_setopt_warn(CURL* curl, int option, ...)
{
    CURLcode result;
    ...
    if (opt < CURLOPTTYPE_OBJECTPOINT)
    {
        ...
        result = qcurl_easy_setopt(curl, opt, longValue);
    }
    else if (opt < CURLOPTTYPE_OFF_T)
    {
        void* pointerValue = va_arg(argp, void*);
        result = qcurl_easy_setopt(curl, opt, pointerValue);
    }
    else
    {
        curl_off_t offsetValue = va_arg(argp, curl_off_t);
        result = qcurl_easy_setopt(curl, opt, offsetValue);
    }
...
void CL_cURL_BeginDownload(const char* localName, const char* remoteURL)
{
    ...
    qcurl_easy_setopt_warn(
        clc.downloadCURL, CURLOPT_PROTOCOLS, CURLPROTO_HTTP | CURLPROTO_HTTPS | CURLPROTO_FTP | CURLPROTO_FTPS);
