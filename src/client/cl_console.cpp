...
// Reduced from 163840 to 32768 to prevent stack overflow on Windows
// See: https://github.com/GrangerHub/tremulous/issues/116
    ...
    else
    {
        ...
        for (i = 0; i < numlines; i++)
        {
            for (j = 0; j < numchars; j++)
            {
                ...
                Vector4Copy(tcbuf[((con.current - i + oldtotallines) % oldtotallines) * oldwidth + j],
...
void Con_Linefeed(void)
{
    ...
    for (i = 0; i < con.linewidth; i++)
    {
        ...
        Vector4Copy(
            g_color_table[ColorIndex(COLOR_WHITE)], con.text_color[(con.current % con.totallines) * con.linewidth + i]);
    }
}
...
static void Con_DrawInput(void)
{
    ...
    Field_Draw(&g_consoleField, con.xadjust + 2 * SMALLCHAR_WIDTH, y, SCREEN_WIDTH - 3 * SMALLCHAR_WIDTH, true, true);
}
...
void Con_DrawSolidConsole(float frac)
{
    ...
    {
        SCR_DrawSmallChar(
            cls.glconfig.vidWidth - (i - x + 1) * SMALLCHAR_WIDTH, lines - SMALLCHAR_HEIGHT, Q3_VERSION[x]);
    }
    ...
    for (i = 0; i < rows; i++, y -= SMALLCHAR_HEIGHT, row--)
    {
        ...
        for (x = 0; x < con.linewidth; x++)
        {
            ...
            if (!Vector4Compare(currentColor, text_color[x]))
            {
                ...
                currentLuminance =
                    sqrt((0.299 * currentColor[0] * currentColor[0]) + (0.587 * currentColor[1] * currentColor[1]) +
...
void Con_DrawConsole(void)
{
    ...
    // draw the chat line
    if (clc.netchan.alternateProtocol == 2 && (Key_GetCatcher() & KEYCATCH_MESSAGE))
    {
        int skip;
        ...
        Field_BigDraw(&chatField, skip * BIGCHAR_WIDTH, 232, SCREEN_WIDTH - (skip + 1) * BIGCHAR_WIDTH, true, true);
    }
