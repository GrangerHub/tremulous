...
static void BG_Bucket_Resize_Array_Of_Buckets(unsigned int new_num_buckets) {
    ...
    if(new_num_buckets > 0) {
        buckets = (bucket_t *)BG_Alloc(new_num_buckets * sizeof(bucket_t));
...
unsigned int BG_Bucket_Create_Bucket(void) {
    ...
    BG_Bucket_Resize_Array_Of_Buckets(num_buckets + 1);
...
void BG_Bucket_Delete_Bucket(unsigned int bucket_handle) {
    assert(
        ...
        "BG_Bucket_Destroy_Bucket: bucket handle not found");

    buckets[bucket_handle].bucket_slot_used = qfalse;
}

void BG_Bucket_Destroy_All_Buckets(void) {
    BG_Bucket_Resize_Array_Of_Buckets(0);
}

static void BG_Bucket_Resize_Bucket(
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "BG_Bucket_Resize_Bucket: bucket handle not found");
    ...
    for(i = 0; i < NUM_BUCKET_LISTS; i++) {
        ...
        (void **)BG_Alloc(new_bucket_size * sizeof(void*));
...
static void BG_Bucket_Add_Item_To_List(
...
static void BG_Bucket_Remove_Item_By_Index_From_List(
...
void BG_Bucket_Add_Item_To_Bucket(
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "BG_Bucket_Add_Item_To_Bucket: bucket handle not found");

    BG_Bucket_Resize_Bucket(
        bucket_handle, buckets[bucket_handle].bucket_size + 1);

    BG_Bucket_Add_Item_To_List(
...
void BG_Bucket_Remove_Item_From_Bucket(
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "BG_Bucket_Remove_Item_From_Bucket: bucket handle not found");
    assert(
        buckets[bucket_handle].bucket_size > 0 &&
        "BG_Bucket_Remove_Item_From_Bucket: bucket is empty");
    ...
    assert(
        item_found &&
        "BG_Bucket_Remove_Item_From_Bucket: item not found");
}
...
void* BG_Bucket_Select_A_Random_Item(unsigned int bucket_handle) {
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "BG_Bucket_Select_A_Random_Item: bucket handle not found");
    assert(
        buckets[bucket_handle].bucket_size > 0 &&
        "BG_Bucket_Select_A_Random_Item: bucket is empty");
    ...
    BG_Bucket_Remove_Item_By_Index_From_List(
...
void  BG_Bucket_Select_A_Specific_Item(unsigned int bucket_handle, void* item) {
    ...
    assert(selected_item && "BG_Bucket_Select_A_Specific_Item: item not found");
