...
==============
*/
classConfig_t *BG_ClassConfig(class_t class) { return &bg_classConfigList[class]; }
...
qboolean BG_AlienCanEvolve(class_t class, int credits, int stage)
{
    ...
    for (i = 0; i < bg_numClasses; i++)
    {
        ...
        for (j = 0; j < 3; j++)
        {
            ...
            if (tclass != PCL_NONE && BG_ClassAllowedInStage(tclass, stage) && BG_ClassIsAllowed(tclass) &&
...
========================
*/
void BG_GetConflictingWithInventory(
    int refSlots, int stats[], int conflictingWP[WP_NUM_WEAPONS - 1], int conflictingUP[UP_NUM_UPGRADES - 1])
{
    int i, j = 0;
...
========================
*/
qboolean BG_InventoryContainsUpgrade(int item, int stats[]) { return (stats[STAT_ITEMS] & (1 << item)); }
...
========================
*/
qboolean BG_UpgradeIsActive(int item, int stats[]) { return (stats[STAT_ACTIVEITEMS] & (1 << item)); }
...
unsigned int BG_Bucket_Create_Bucket(void) { return Q_Bucket_Create_Bucket(BG_Alloc, BG_Free); }

void BG_Bucket_Delete_Bucket(unsigned int bucket_handle) { Q_Bucket_Delete_Bucket(bucket_handle); }

void BG_Bucket_Destroy_All_Buckets(void) { Q_Bucket_Destroy_All_Buckets(BG_Alloc, BG_Free); }

void BG_Bucket_Add_Item_To_Bucket(unsigned int bucket_handle, void *item)
{
    Q_Bucket_Add_Item_To_Bucket(bucket_handle, item, BG_Alloc, BG_Free);
}

void BG_Bucket_Remove_Item_From_Bucket(unsigned int bucket_handle, void *item)
{
    Q_Bucket_Remove_Item_From_Bucket(bucket_handle, item, BG_Alloc, BG_Free);
}
