...
==================
*/
int G_ClientNumbersFromString(char *s, int *plist, int max, qboolean alphanumeric)
{
    gclient_t *p;
    int i, found = 0;
    char *s_start = s;
        ...
        if (traceEnt->health > 0)
        {
            if (!deconstruct)
            {
                G_Damage(traceEnt, ent, ent, forward, tr.endpos, traceEnt->health, 0, MOD_SUICIDE);
                     ...
                     (ent->client->pers.teamSelection != TEAM_HUMANS || G_FindPower(traceEnt, qtrue) || lastSpawn))
            {
...
=================
*/
static qboolean G_SellClientItems(
    gentity_t *ent, int conflictingWP[WP_NUM_WEAPONS - 1], int conflictingUP[UP_NUM_UPGRADES - 1])
{
    int i;
...
void Cmd_Buy_f(gentity_t *ent)
{
    ...
    if (weapon != WP_NONE)
    {
        ...
        // can afford this?
        if (BG_Weapon(weapon)->price >
            (short)ent->client->pers.credit + G_PriceOfClientItems(conflictingWP, conflictingUP))
        {
...
static void Cmd_RotateBuild(gentity_t *ent, int unit, qboolean add)
{
    ...
    if (add == qtrue)
    {
        currentAngle = (ent->client->ps.stats[STAT_BUILDABLE] & SB_ROTATION_MASK) / SB_ROTATION_UNIT;
...
=================
*/
void Cmd_RotateBuildLeft_f(gentity_t *ent) { Cmd_RotateBuild(ent, -1, qtrue); }
...
=================
*/
void Cmd_RotateBuildRight_f(gentity_t *ent) { Cmd_RotateBuild(ent, 1, qtrue); }
...
commands_t cmds[] = {{"a", CMD_MESSAGE | CMD_INTERMISSION, Cmd_AdminMessage_f},
    ...
    {"specBuilderTeam", CMD_SPEC, Cmd_SpecBuilderTeam_f}, {"specLayoutClear", CMD_SPEC, Cmd_SpecLayoutClear_f},
    {"specLayoutDelete", CMD_SPEC, Cmd_SpecLayoutDelete_f}, {"specLayoutEnable", CMD_SPEC, Cmd_SpecLayoutEnable_f},
    {"specLayoutList", CMD_SPEC, Cmd_SpecLayoutList_f}, {"specLayoutLoad", CMD_SPEC, Cmd_SpecLayoutLoad_f},
    {"specLayoutRate", CMD_SPEC, Cmd_SpecLayoutRate_f}, {"specLayoutRename", CMD_SPEC, Cmd_SpecLayoutRename_f},
    {"specLayoutSave", CMD_SPEC, Cmd_SpecLayoutSave_f}, {"specLayoutSort", CMD_SPEC, Cmd_SpecLayoutSort_f},
    {"specLayoutVote", CMD_SPEC, Cmd_SpecLayoutVote_f}, {"specLayoutVoteList", CMD_SPEC, Cmd_SpecLayoutVoteList_f},
