...
struct gentity_s {
    ...
    int specBuildOwner;  // clientNum of owner (-1 if not spec build)
};
...
typedef struct {
    ...
    // Spectator Builder Layouts
    int specBuilderTeam;  // 0=Humans, 1=Aliens, 2=Both
} clientPersistant_t;
...
typedef enum { TW_NOT = 0, TW_IMMINENT, TW_PASSED } timeWarning_t;

// fate of a buildable
typedef enum { BF_CONSTRUCT, BF_DECONSTRUCT, BF_REPLACE, BF_DESTROY, BF_TEAMKILL, BF_UNPOWER, BF_AUTO } buildFate_t;
...
itemBuildError_t G_CanBuild(gentity_t *ent, buildable_t buildable, int distance, vec3_t origin, vec3_t normal,
    vec3_t angles, int *groundEntNum);
...
// Spectator Builder Layouts
#define MAX_SPECLAYOUTS_PER_MAP 128
void Cmd_SpecLayoutList_f(gentity_t *ent);
void Cmd_SpecLayoutSave_f(gentity_t *ent);
void Cmd_SpecLayoutLoad_f(gentity_t *ent);
void Cmd_SpecLayoutDelete_f(gentity_t *ent);
void Cmd_SpecLayoutEnable_f(gentity_t *ent);
void Cmd_SpecLayoutRename_f(gentity_t *ent);
void Cmd_SpecLayoutRate_f(gentity_t *ent);
void Cmd_SpecLayoutClear_f(gentity_t *ent);
void Cmd_SpecLayoutSort_f(gentity_t *ent);
void Cmd_SpecLayoutVote_f(gentity_t *ent);
void Cmd_SpecLayoutVoteList_f(gentity_t *ent);
void Cmd_SpecBuilderTeam_f(gentity_t *ent);
...
void trap_LocateGameData(
    gentity_t *gEnts, int numGEntities, int sizeofGEntity_t, playerState_t *gameClients, int sizeofGameClient);
