...
void RB_BeginDrawingView(void)
{
    ...
    {
        if (r_fastsky->integer == 2)
        {
            // r_fastsky 2: Draw solid green skybox instead of clearing to black
            // Don't add GL_COLOR_BUFFER_BIT, we'll draw the green skybox after clearing depth
        }
        else
        {
            // r_fastsky 1: Clear to black (original fastsky behavior)
    ...
    // r_fastsky 2: Draw solid green skybox
        ...
        if (cmd->cubeSide != -1)
        {
            if (tr.shadowCubemaps[cmd->map])
            {
                qglCopyTextureSubImage2DEXT(tr.shadowCubemaps[cmd->map]->texnum,
                    ...
                    glConfig.vidHeight - (backEnd.refdef.y + PSHADOW_MAP_SIZE), PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE);
            }
        }
        else
        {
...
const void *RB_PostProcess(const void *data)
{
    ...
    if (tr.msaaResolveFbo)
    {
        ...
        FBO_FastBlit(
            tr.renderFbo, NULL, tr.msaaResolveFbo, NULL, GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT, GL_NEAREST);
    ...
    if (r_ssao->integer)
    {
        ...
        FBO_Blit(
            tr.screenSsaoFbo, srcBox, NULL, srcFbo, dstBox, NULL, NULL, GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ZERO);
    }
    ...
    // Apply gamma correction as final post-processing pass
    // (replaces hardware gamma ramp removed in SDL3)
    RB_GammaCorrection(srcFbo, srcBox, dstBox);
