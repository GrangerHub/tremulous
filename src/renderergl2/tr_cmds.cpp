...
=============
*/
void RE_BackgroundBlur ( float x, float y, float w, float h, float amount) {
	backgroundBlurCommand_t	*cmd;

	if (!tr.registered)
		return;

	if ( !( cmd = (backgroundBlurCommand_t*)R_GetCommandBuffer(sizeof( *cmd )) ) )
		return;

	cmd->commandId = RC_BACKGROUNDBLUR;
	cmd->x = x;
	cmd->y = y;
	cmd->w = w;
	cmd->h = h;
