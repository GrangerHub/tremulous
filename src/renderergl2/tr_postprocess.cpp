...
void RB_GaussianBlur(float blur)
{
...

    {
        ...
        FBO_BlitFromTexture(
            tr.whiteImage, NULL, NULL, tr.textureScratchFbo[0], NULL, NULL, color, GLS_DEPTHTEST_DISABLE);
...
Applies gamma correction as a final post-processing pass.
SDL3 removed hardware gamma ramp support, so this shader-based
approach replaces GLimp_SetGamma().
=============
*/
void RB_GammaCorrection(FBO_t *srcFbo, ivec4_t srcBox, ivec4_t dstBox)
{
    ...
    // Apply gamma correction as a final post-processing shader pass.
    // Skip if gamma is at or near default (no correction needed).
    if (fabs(r_gamma->value - 1.0f) < 0.001f)
        return;

    // FBO_Blit cannot read from the default framebuffer (NULL).
    // Copy the current frame from NULL to srcFbo, then apply gamma
    // from srcFbo back to NULL.
    FBO_FastBlit(NULL, dstBox, srcFbo, NULL, GL_COLOR_BUFFER_BIT, GL_NEAREST);

    // Set gamma uniform (works via DSA even before program is bound)
    GLSL_SetUniformFloat(&tr.gammaShader, UNIFORM_GAMMA, r_gamma->value);

    // Set intensity uniform (optional, defaults to 1.0 if not used)
    // This allows shader-based intensity scaling instead of CPU-side lookup tables
    GLSL_SetUniformFloat(&tr.gammaShader, UNIFORM_INTENSITY, r_intensity->value);

    // Apply gamma correction: output = pow(input, 1.0 / gamma)
    VectorSet4(color, 1.0f, 1.0f, 1.0f, 1.0f);
    FBO_Blit(srcFbo, NULL, NULL, NULL, dstBox, &tr.gammaShader, color, 0);
}
