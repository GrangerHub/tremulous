...
void R_Register( void )
{
	...
	ri.Cvar_SetDescription(r_width, "Custom width to use with \\r_mode -1.");
	r_height = ri.Cvar_Get( "r_height", "0", CVAR_ARCHIVE | CVAR_LATCH );
	ri.Cvar_SetDescription(r_height, "Custom height to use with \\r_mode -1.");
	...
	ri.Cvar_SetDescription(r_flares, "Enable corona effects on light sources.");
	...
	ri.Cvar_SetDescription(r_debugLight, "Debuggign tool to print ambient and directed lighting information.");
	...
	ri.Cvar_SetDescription(r_drawentities, "Draw all world entites.");
	...
	ri.Cvar_SetDescription(r_lockpvs, "Debugging tool: Locks to current potentially visible set. Useful for texting vis-culling in maps.");
