...
void RB_ClipSkyPolygons(shaderCommands_t *input)
{
    ...
    for (i = 0; i < input->numIndexes; i += 3)
    {
        for (j = 0; j < 3; j++)
        {
            VectorSubtract(input->xyz[input->indexes[i + j]], backEnd.viewParms.orientation.origin, p[j]);
        }
...
Draws a solid green skybox cube for r_fastsky 2
================
*/
void RB_DrawGreenSkyBox(void)
{
    vec3_t origin;
    float size = 10000.0f;  // Large size to encompass the entire view

    VectorCopy(backEnd.viewParms.orientation.origin, origin);

    GL_State(0);
    GL_Cull(CT_FRONT_SIDED);

    // Use immediate mode for simplicity (same as GL1)
    {
        mat4_t trans, modelview;

        Mat4Translation(origin, trans);
        Mat4Multiply(glState.modelview, trans, modelview);
        GL_SetModelviewMatrix(modelview);

        qglColor3f(0.0f, 1.0f, 0.0f);  // Pure green

        qglBegin(GL_QUADS);

        // Top face (y = +size)
        qglVertex3f(-size, size, -size);
        qglVertex3f(-size, size, size);
        qglVertex3f(size, size, size);
        qglVertex3f(size, size, -size);
