...
enum { GLSL_INT, GLSL_FLOAT, GLSL_FLOAT5, GLSL_VEC2, GLSL_VEC3, GLSL_VEC4, GLSL_MAT16 };
...
void R_SubdividePatchToGrid(
    srfBspSurface_t *grid, int width, int height, srfVert_t points[MAX_PATCH_SIZE * MAX_PATCH_SIZE]);
...
vao_t *R_CreateVao(
    const char *name, byte *vertexes, int vertexesSize, byte *indexes, int indexesSize, vaoUsage_t usage);
...
void VaoCache_CheckAdd(
    bool *endSurface, bool *recycleVertexBuffer, bool *recycleIndexBuffer, int numVerts, int numIndexes);
...
void RE_SaveJPG(
    char *filename, int quality, int image_width, int image_height, unsigned char *image_buffer, int padding);
size_t RE_SaveJPGToBuffer(
    byte *buffer, size_t bufSize, int quality, int image_width, int image_height, byte *image_buffer, int padding);
