...
===============
*/
static bool RawImage_ScaleToPower2(
    byte **data, int *inout_width, int *inout_height, imgType_t type, int /*imgFlags_t*/ flags, byte **resampledBuffer)
{
...
static GLenum RawImage_GetFormat(
    const byte *data, int numPixels, GLenum picFormat, bool lightMap, imgType_t type, int /*imgFlags_t*/ flags)
{
    int samples = 3;
...
static void RawImage_UploadToRgtc2Texture(GLuint texture, int miplevel, int x, int y, int width, int height, byte *data)
{
    ...
    qglCompressedTextureSubImage2DEXT(
        texture, GL_TEXTURE_2D, miplevel, x, y, width, height, GL_COMPRESSED_RG_RGTC2, size, compressedData);
...
static void RawImage_UploadTexture(GLuint texture, byte *data, int x, int y, int width, int height, GLenum target,
    GLenum picFormat, int numMips, GLenum internalFormat, imgType_t type, int /*imgFlags_t*/ flags, bool subtexture)
{
...
static void Upload32(
    byte *data, int x, int y, int width, int height, GLenum picFormat, int numMips, image_t *image, bool scaled)
{
    int i, c;
    byte *scan;
    ...
    if (cubemap)
    {
        for (i = 0; i < 6; i++)
        {
            ...
            RawImage_UploadTexture(image->texnum, data, x, y, width, height, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i,
                picFormat, numMips, internalFormat, type, flags, false);
            for (c = numMips; c; c--)
            {
    ...
    else
    {
        RawImage_UploadTexture(image->texnum, data, x, y, width, height, GL_TEXTURE_2D, picFormat, numMips,
            internalFormat, type, flags, false);
    }
    ...
    imgType_t type, int /*imgFlags_t*/ flags, int internalFormat)
{
    ...
    switch (internalFormat)
    {
        ...
        case GL_DEPTH_COMPONENT32_ARB:
            ...
            break;
        default:
            qglTextureParameterfEXT(
                image->texnum, textureTarget, GL_TEXTURE_MIN_FILTER, mipmap ? gl_filter_min : GL_LINEAR);
            qglTextureParameterfEXT(
                image->texnum, textureTarget, GL_TEXTURE_MAG_FILTER, mipmap ? gl_filter_max : GL_LINEAR);
            break;
    }
...
image_t *R_CreateImage(
    const char *name, byte *pic, int width, int height, imgType_t type, int /*imgFlags_t*/ flags, int internalFormat)
{
...
void R_CreateBuiltinImages(void)
{
    ...
    if (glRefConfig.framebufferObject)
    {
        ...
        //IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_INTENSITY32F_ARB);
        if (r_shadowBlur->integer || r_ssao->integer)
            tr.hdrDepthImage = R_CreateImage("*hdrDepth", NULL, width, height, IMGTYPE_COLORALPHA,
        ...
        tr.textureDepthImage = R_CreateImage("*texturedepth", NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE,
            IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT24_ARB);

        {
            ...
            tr.calcLevelsImage = R_CreateImage(
                "*calcLevels", p, 1, 1, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
            tr.targetLevelsImage = R_CreateImage(
                "*targetLevels", p, 1, 1, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
            tr.fixedLevelsImage = R_CreateImage(
                "*fixedLevels", p, 1, 1, IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, hdrFormat);
        }
        ...
        {
            tr.pshadowMaps[x] = R_CreateImage(va("*shadowmap%i", x), NULL, PSHADOW_MAP_SIZE, PSHADOW_MAP_SIZE,
        ...
        if (r_sunlightMode->integer)
        {
            for (x = 0; x < 4; x++)
            {
                tr.sunShadowDepthImage[x] =
                    R_CreateImage(va("*sunshadowdepth%i", x), NULL, r_shadowMapSize->integer, r_shadowMapSize->integer,
                        IMGTYPE_COLORALPHA, IMGFLAG_NO_COMPRESSION | IMGFLAG_CLAMPTOEDGE, GL_DEPTH_COMPONENT24_ARB);
                qglTextureParameterfEXT(
                    tr.sunShadowDepthImage[x]->texnum, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_MODE, GL_COMPARE_R_TO_TEXTURE);
                qglTextureParameterfEXT(
                    tr.sunShadowDepthImage[x]->texnum, GL_TEXTURE_2D, GL_TEXTURE_COMPARE_FUNC, GL_LEQUAL);
            }
        ...
        {
            tr.renderCubeImage =
                R_CreateImage("*renderCube", NULL, r_cubemapSize->integer, r_cubemapSize->integer, IMGTYPE_COLORALPHA,
...
void R_SetColorMappings(void)
{
    ...
    // SDL3 removed hardware gamma ramp support. Gamma is now applied once via
    // shader-based post-processing (RB_GammaCorrection) to avoid double gamma,
    // the texture-level gamma table is built as an identity mapping.
    for (i = 0; i < 256; i++)
    {
        s_gammatable[i] = i;
    }
