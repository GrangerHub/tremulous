...
#ifdef _MSC_VER
#include <io.h>
// Modern MSVC (VS2010+) ships <stdint.h>; the manual typedefs below
// were a workaround for pre-2010 MSVC. They now conflict (C2371) when
// <stdint.h> is pulled in via another header, because the manual
// typedefs use __int8 (compiler-specific keyword) while <stdint.h>
// uses `signed char` — same width, different basic type to MSVC.
#include <stdint.h>
...
/* FILESYSTEM */
enum FS_Mode { FS_READ, FS_WRITE, FS_APPEND, FS_APPEND_SYNC };

enum FS_Origin { FS_SEEK_CUR, FS_SEEK_END, FS_SEEK_SET };
...
#define Q_IsShortHexColor(p)                                                                                        \
    ((p) && *(p) == Q_COLOR_HEX_ESCAPE && *((p) + 1) && isalnum(*((p) + 1)) && *((p) + 2) && isalnum(*((p) + 2)) && \
        *((p) + 3) && isalnum(*((p) + 3)))
#define Q_IsLongHexColor(p)                                                                                            \
    ((p) && *(p) == Q_COLOR_HEX_ESCAPE && *((p) + 1) && *((p) + 1) == Q_COLOR_HEX_ESCAPE && *((p) + 2) &&              \
        isalnum(*((p) + 2)) && *((p) + 3) && isalnum(*((p) + 3)) && *((p) + 4) && isalnum(*((p) + 4)) && *((p) + 5) && \
        isalnum(*((p) + 5)) && *((p) + 6) && isalnum(*((p) + 6)) && *((p) + 7) && isalnum(*((p) + 7)))
#define Q_IsHexColor(p) (Q_IsShortHexColor(p) || Q_IsLongHexColor(p))
#define Q_IsHardcodedColor(p) \
    ((p) && *(p) == Q_COLOR_ESCAPE && *((p) + 1) && *((p) + 1) != Q_COLOR_ESCAPE && (isalnum(*((p) + 1))))
#define Q_IsColorString(p)                                                          \
    ((p) && *(p) == Q_COLOR_ESCAPE && *((p) + 1) && *((p) + 1) != Q_COLOR_ESCAPE && \
        (Q_IsHardcodedColor(p) ||                                                   \
            Q_IsHexColor(p + 1)))  // ^[0-9a-zA-Z] or for custom short hex ^#[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F] or custom
                                   // long hex ^##[0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F][0-9a-fA-F]
...
#define VectorLerp2(f, s, e, r)                                                            \
    ((r)[0] = (s)[0] + (f) * ((e)[0] - (s)[0]), (r)[1] = (s)[1] + (f) * ((e)[1] - (s)[1]), \
        (r)[2] = (s)[2] + (f) * ((e)[2] - (s)[2]))
...
#define Vector4Lerp(f, s, e, r)                                                            \
    ((r)[0] = (s)[0] + (f) * ((e)[0] - (s)[0]), (r)[1] = (s)[1] + (f) * ((e)[1] - (s)[1]), \
        (r)[2] = (s)[2] + (f) * ((e)[2] - (s)[2]), (r)[3] = (s)[3] + (f) * ((e)[3] - (s)[3]))
...
void Q_ApproxStrHexColors(
    const char *in_string, char *out_string, const size_t in_string_length, const size_t out_string_length);
...
unsigned int Q_Bucket_Create_Bucket(void *(*qalloc)(int size), void (*qfree)(void *ptr));
void Q_Bucket_Delete_Bucket(unsigned int bucket_handle);
void Q_Bucket_Destroy_All_Buckets(void *(*qalloc)(int size), void (*qfree)(void *ptr));
void Q_Bucket_Add_Item_To_Bucket(
    unsigned int bucket_handle, void *item, void *(*qalloc)(int size), void (*qfree)(void *ptr));
...
typedef enum { DS_NONE, DS_PLAYBACK, DS_RECORDING, DS_NUM_DEMO_STATES } demoState_t;
