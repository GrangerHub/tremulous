...
void VM_Init(void)
{
    // On builds without a JIT compiler (NO_VM_COMPILED), the QVM interpreter
    // is the only QVM option -- it is slower than native and also fragile if
    // the on-disk .qvm is stale (different syscall enum).  This applies to
    // MSVC builds AND MinGW cross-compiles (both define NO_VM_COMPILED).
    // Default to VMI_NATIVE (0) so the freshly-built game/cgame/ui DLLs are
    // used.  Users can still override via the console or q3config.cfg.
...
============
*/
__attribute__((no_sanitize_address)) intptr_t QDECL VM_DllSyscall(intptr_t arg, ...)
{
...
void VM_ClearCallLevel(vm_t *vm) { vm->callLevel = 0; }
...
==============
*/
__attribute__((no_sanitize_address)) intptr_t QDECL VM_Call(vm_t *vm, int callnum, ...)
{
    vm_t *oldVM;
    intptr_t r;
