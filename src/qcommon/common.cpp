...
void Com_Colors_f(void)
{
    ...
    for (row = 0; row < ((int)ceilf(((float)color_definitions_length) / 2.0f)); row++)
    {
        ...
        {
            Com_Printf(
                " ^%s%-20s ^^%-5s", color_definitions[i].code, color_definitions[i].name, color_definitions[i].code);
        }
        Com_Printf("^7\n");
    }
    ...
    Com_Printf(
        "^3The format for the hardcoded standard color codes is ^5^^x^3, where x is alphanumeric "
        "(^7[^50-9^7][^5a-z^7][^5A-Z^7]^3).\n\n");
    Com_Printf(
        "^3The short format for custom hexadecimal defined color codes is ^5^^#xxx^3, where x is a hexadecimal "
        "(^7[^50-9^7][^5a-f^7][^5A-F^7]^3).\n\n");
    Com_Printf(
        "^3The long format for custom hexadecimal defined color codes (which has even more possible colors) is "
        "^5^^##xxxxxx^3, where x is a hexadecimal (^7[^50-9^7][^5a-f^7][^5A-F^7]^3).^7\n\n\n");
}
...
#ifdef HUNK_DEBUG
        Com_sprintf(
            buf, sizeof(buf), "size = %8d: %s, line: %d (%s)\r\n", block->size, block->file, block->line, block->label);
        FS_Write(buf, strlen(buf), logfile);
#endif
        size += block->size;
        numBlocks++;
...
static bool Field_Complete(void)
{
    ...
    Q_strncpyz(
        &completionField->buffer[completionOffset], shortestMatch, sizeof(completionField->buffer) - completionOffset);
...
void Field_ListCompletion(char *listJson, void (*callback)(const char *s))
{
    ...
    // JSON parse array
    for (arrayPtr = JSON_ArrayGetFirstValue(listJson, listEnd); arrayPtr;
...
static void *Z_PlaceHolderAlloc(int size) { return Z_Malloc(size); }

unsigned int Com_Bucket_Create_Bucket(void) { return Q_Bucket_Create_Bucket(Z_PlaceHolderAlloc, Z_Free); }

void Com_Bucket_Delete_Bucket(unsigned int bucket_handle) { Q_Bucket_Delete_Bucket(bucket_handle); }

void Com_Bucket_Destroy_All_Buckets(void) { Q_Bucket_Destroy_All_Buckets(Z_PlaceHolderAlloc, Z_Free); }

void Com_Bucket_Add_Item_To_Bucket(unsigned int bucket_handle, void *item)
{
    Q_Bucket_Add_Item_To_Bucket(bucket_handle, item, Z_PlaceHolderAlloc, Z_Free);
}

void Com_Bucket_Remove_Item_From_Bucket(unsigned int bucket_handle, void *item)
{
    Q_Bucket_Remove_Item_From_Bucket(bucket_handle, item, Z_PlaceHolderAlloc, Z_Free);
}
