...
typedef struct bucket_s
{
    ...
    bool           bucket_slot_used;
} bucket_t;

static bucket_t*    buckets = NULL;
static unsigned int num_buckets = 0;

static void Com_Bucket_Resize_Array_Of_Buckets(unsigned int new_num_buckets) {
    ...
    if(new_num_buckets > 0) {
        buckets = (bucket_t *)Z_Malloc(new_num_buckets * sizeof(bucket_t));
...
unsigned int Com_Bucket_Create_Bucket(void) {
    ...
    Com_Bucket_Resize_Array_Of_Buckets(num_buckets + 1);
    memset(&buckets[num_buckets - 1], 0, sizeof(bucket_t));
    buckets[num_buckets - 1].bucket_slot_used = true;
...
void Com_Bucket_Delete_Bucket(unsigned int bucket_handle) {
    assert(
        ...
        "Com_Bucket_Destroy_Bucket: bucket handle not found");

    buckets[bucket_handle].bucket_slot_used = false;
}

void Com_Bucket_Destroy_All_Buckets(void) {
    Com_Bucket_Resize_Array_Of_Buckets(0);
}

static void Com_Bucket_Resize_Bucket(
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "Com_Bucket_Resize_Bucket: bucket handle not found");
    ...
    for(i = 0; i < NUM_BUCKET_LISTS; i++) {
        ...
        (void **)Z_Malloc(new_bucket_size * sizeof(void*));
...
static void Com_Bucket_Add_Item_To_List(
...
static void Com_Bucket_Remove_Item_By_Index_From_List(
    ...
    int   list_length;
		...
		item = bucket->list[bucket_list][item_index];
...
void Com_Bucket_Add_Item_To_Bucket(
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "Com_Bucket_Add_Item_To_Bucket: bucket handle not found");

    Com_Bucket_Resize_Bucket(
        bucket_handle, buckets[bucket_handle].bucket_size + 1);

    Com_Bucket_Add_Item_To_List(
...
void Com_Bucket_Remove_Item_From_Bucket(
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "Com_Bucket_Remove_Item_From_Bucket: bucket handle not found");
    assert(
        buckets[bucket_handle].bucket_size > 0 &&
        "Com_Bucket_Remove_Item_From_Bucket: bucket is empty");
    ...
    assert(
        item_found &&
        "Com_Bucket_Remove_Item_From_Bucket: item not found");
}
...
void* Com_Bucket_Select_A_Random_Item(unsigned int bucket_handle) {
    ...
    assert(
        (bucket_handle < num_buckets) &&
        buckets[bucket_handle].bucket_slot_used &&
        "Com_Bucket_Select_A_Random_Item: bucket handle not found");
    assert(
        buckets[bucket_handle].bucket_size > 0 &&
        "Com_Bucket_Select_A_Random_Item: bucket is empty");
    ...
    Com_Bucket_Remove_Item_By_Index_From_List(
...
void Com_Bucket_Select_A_Specific_Item(unsigned int bucket_handle, void* item) {
    ...
    assert(selected_item && "Com_Bucket_Select_A_Specific_Item: item not found");
