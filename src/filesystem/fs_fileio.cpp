...
static qboolean fs_generate_subpath(fsc_stream_t *stream, const char *path, int flags)
{
    ...
    else
    {
        ...
        {
            char current = conversion_table[*(unsigned char *)index];
            if (current == '/' && !(flags & FS_ALLOW_SLASH))
                current = '_';
            if (sanitized_path_length >= sizeof(sanitized_path) - 1)
                continue;
        ...
        if (!Q_stricmp(sanitized_path + sanitized_path_length - 4, ".qvm"))
            return qfalse;
        if (!(flags & FS_ALLOW_PK3) && sanitized_path_length >= 4 &&
            !Q_stricmp(sanitized_path + sanitized_path_length - 4, ".pk3"))
            return qfalse;
        if (!(flags & FS_ALLOW_DLL))
        {
            ...
            if (sanitized_path_length >= 4 && !Q_stricmp(sanitized_path + sanitized_path_length - 4, ".dll"))
                return qfalse;
            if (sanitized_path_length >= 3 && !Q_stricmp(sanitized_path + sanitized_path_length - 3, ".so"))
                return qfalse;
            if (sanitized_path_length >= 6 && !Q_stricmp(sanitized_path + sanitized_path_length - 6, ".dylib"))
                return qfalse;
        }
    ...
    int path2_flags, char *target, unsigned int target_size)
{
    ...
    if (!fs_sourcedirs[source_dir_id].active)
        return 0;
    return fs_generate_path(fs_sourcedirs[source_dir_id].path_cvar->string, path1, path2, FS_NO_SANITIZE, path1_flags,
        path2_flags, target, target_size);
}
...
void FS_HomeRemove(const char *homePath)
{
    ...
    if (!fs_generate_path_writedir(FS_GetCurrentGameDir(), homePath, 0, FS_ALLOW_SLASH, path, sizeof(path)))
    {
...
{
    char path[FS_MAX_PATH];
    if (!fs_generate_path_sourcedir(0, FS_GetCurrentGameDir(), file, 0, FS_ALLOW_SLASH, path, sizeof(path)))
        return qfalse;
...
char *fs_read_data(const fsc_file_t *file, const char *path, unsigned int *size_out)
{
    ...
    if (file)
    {
        ...
        if (cache_entry)
        {
            ...
            if (fs_debug_fileio->integer)
                Com_Printf("result: loaded %u bytes from cache\n", cache_entry->size - 1);
    ...
    if (size_out)
        *size_out = size;
    if (fs_debug_fileio->integer)
        Com_Printf("result: loaded %u bytes from file\n", size);
    return data;

// Free buffer if there was an error extracting data
error:
    if (fs_debug_fileio->integer)
        Com_Printf("result: failed to load file\n");
    if (cache_entry)
    {
        cache_entry->file = 0;
...
{
    if (!data)
        Com_Error(ERR_DROP, "Null parameter to fs_free_data.");
...
fileHandle_t fs_direct_read_handle_open(const fsc_file_t *file, const char *path, unsigned int *size_out)
{
    ...
    if (fs_debug_fileio->integer)
        Com_Printf("********** opening direct read handle **********\npath: %s\n", debug_path);
    ...
    {
        if (fs_debug_fileio->integer)
            Com_Printf("result: failed to open file\n");
        return 0;
    }
...
static fileHandle_t fs_pk3_read_handle_open(const fsc_file_t *file)
{
    ...
    if (fs_debug_fileio->integer)
        Com_Printf("********** opening pk3 read handle **********\nfile: %s\n", debug_path);
...
static fileHandle_t fs_write_handle_open(const char *path, qboolean append, qboolean sync)
{
    ...
    if (fs_debug_fileio->integer)
        Com_Printf("********** opening write handle **********\npath: %s\n", path);

    if (!os_path)
    {
        if (fs_debug_fileio->integer)
            Com_Printf("result: failed to convert to os path\n");
        return 0;
    }
...
fileHandle_t FS_FCreateOpenPipeFile(const char *filename)
{
    ...
    if (fs_generate_path_writedir(
            FS_GetCurrentGameDir(), filename, 0, FS_ALLOW_SLASH | FS_CREATE_DIRECTORIES_FOR_FILE, path, sizeof(path)))
    {
...
void fs_print_handle_list(void)
{
    ...
    for (i = 0; i < MAX_HANDLES; ++i)
    {
        if (!handles[i])
            continue;
        Com_Printf("********** handle %i **********\ntype: %s\nowner: %s\npath: %s\n", i + 1,
            identify_handle_type(handles[i]->type), identify_handle_owner(handles[i]->owner), handles[i]->debug_path);
    }
}
    ...
    for (i = 0; i < MAX_HANDLES; ++i)
    {
        if (handles[i] && handles[i]->owner == owner)
        {
            Com_Printf(
                ...
                "type: %s\nowner: %s\npath: %s\n*****************\n",
                identify_handle_type(handles[i]->type), identify_handle_owner(handles[i]->owner),
...
static long open_index_read_handle(
    const char *filename, fileHandle_t *handle, int lookup_flags, qboolean allow_direct_handle)
{
...
static fileHandle_t open_write_handle_path_gen(
    const char *mod_dir, const char *path, qboolean append, qboolean sync, int flags)
{
    ...
    if (!fs_generate_path_writedir(mod_dir, path, FS_CREATE_DIRECTORIES,
            FS_ALLOW_SLASH | FS_CREATE_DIRECTORIES_FOR_FILE | flags, full_path, sizeof(full_path)))
        return 0;
    ...
    if (mode == FS_READ)
    {
        if (owner != FS_HANDLEOWNER_SYSTEM)
        {
                ...
                fs_connected_server_pure_state() != 1)
            {
                ...
                char path[FS_MAX_PATH];
                if (fs_generate_path_writedir(FS_GetCurrentGameDir(), qpath, 0, FS_ALLOW_SLASH, path, sizeof(path)))
                {
            ...
            if (!handle)
                size = open_index_read_handle(qpath, f ? &handle : 0, lookup_flags, qtrue);
        }

        // Engine reads don't do anything fancy so just use the basic method
        else
            size = open_index_read_handle(qpath, f ? &handle : 0, 0, qfalse);
...
int FS_Read(void *buffer, int len, fileHandle_t f) { return fs_handle_read((char *)buffer, len, f); }
...
void FS_SV_Rename(const char *from, const char *to, qboolean safe)
{
    ...
    if (!fs_generate_path_writedir(from, 0, FS_ALLOW_SLASH, 0, source_path, sizeof(source_path)))
        return;
    if (!fs_generate_path_writedir(
            to, 0, FS_ALLOW_SLASH | FS_CREATE_DIRECTORIES_FOR_FILE, 0, target_path, sizeof(target_path)))
        return;
...
void FS_FreeFile(void *buffer) { fs_free_data((char *)buffer); }
