...
#define IS_ALPHANUMERIC(c) (((c) >= 'a' && (c) <= 'z') || ((c) >= 'A' && (c) <= 'Z') || ((c) >= '0' && (c) <= '9'))
    ...
    {
        if (IS_ALPHANUMERIC(sanitized_path[i]))
            break;
        sanitized_path[i] = '_';
    }
...
static int fs_open_index_read_handle(
...
static fileHandle_t fs_open_write_handle_path_gen(
    ...
    if (mode == FS_READ)
    {
        if (owner != FS_HANDLEOWNER_SYSTEM)
        {
            ...
            if (((lookup_flags & (LOOKUPFLAG_IGNORE_PURE_LIST | LOOKUPFLAG_PURE_ALLOW_DIRECT_SOURCE)) ||
                ...
                !COM_CompareExtension(qpath, ".arena") && !COM_CompareExtension(qpath, ".bot"))
            {
                ...
                if (fs_generate_path_writedir(
                        FS_GetCurrentGameDir(), qpath, 0, FS_ALLOW_DIRECTORIES, path, sizeof(path)))
                {
            ...
            if (!handle)
                size = fs_open_index_read_handle(qpath, f ? &handle : 0, lookup_flags, qtrue);
        }

        // Engine reads don't do anything fancy so just use the basic method
        else
            size = fs_open_index_read_handle(qpath, f ? &handle : 0, 0, qfalse);
