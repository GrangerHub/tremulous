...
static file_list_sort_key_t *generate_sort_key(
    const fsc_file_t *file, fsc_stack_t *stack, const filelist_query_t *query)
{
    char buffer[1024];
...
static void temp_file_set_insert(
    fs_hashtable_t *ht, const fsc_file_t *file, file_list_sort_key_t *sort_key, const char *path)
{
...
static qboolean check_path_enabled(fsc_stream_t *stream, const filelist_work_t *flw)
{
    ...
    {
        if (stream->position < flw->extension_length)
            return qfalse;
        if (Q_stricmpn(
                stream->data + stream->position - flw->extension_length, flw->query->extension, flw->extension_length))
            return qfalse;
    }
    if (flw->query->filter)
    {
        if (!Com_FilterPath((char *)flw->query->filter, stream->data, qfalse))
            return qfalse;
    }
    return qtrue;
}
...
static qboolean check_file_enabled(const fsc_file_t *file, const filelist_work_t *flw)
{
    ...
    if (flw->query->flags & FL_FLAG_USE_PURE_LIST)
        disabled_flags |= FD_FLAG_CHECK_PURE;
    if (fs_file_disabled(file, disabled_flags))
        return qfalse;
    if ((flw->query->flags & FL_FLAG_IGNORE_TAPAK0) && file->sourcetype == FSC_SOURCETYPE_PK3 &&
        ((fsc_file_direct_t *)STACKPTR(((fsc_file_frompk3_t *)file)->source_pk3))->pk3_hash == 2430342401u)
        return qfalse;
    return qtrue;
}
            ...
            for (i = flw->crop_length; i < path_stream.position; ++i)
            {
                if (path_stream.data[i] == '/')
                {
                    ...
                    if (depth < flw->directory_depth)
                    {
                        ...
                        {
                            if (output->element_count >= MAX_FOUND_FILES)
                                return;
                            if (!sort_key)
                                sort_key = generate_sort_key(file, &flw->temp_stack, flw->query);
...
static fsc_directory_t *get_start_directory(const char *path, int length)
{
    ...
    while ((directory_ptr = fsc_hashtable_next(&hti)))
    {
        ...
        if (path)
        {
            if (!Q_stricmp((const char *)STACKPTR(directory->qp_dir_ptr), path2))
                break;
        }
        else
        {
...
static char **list_files(const char *path, int *numfiles_out, filelist_query_t *query)
{
    ...
    if (start_directory)
    {
        ...
        if (query->filter)
            flw.file_depth = flw.directory_depth = 256;
        if (flw.extension_length)
        {
            // Optimization to skip processing path types blocked by extension anyway
            if (query->extension[flw.extension_length - 1] == '/')
                flw.file_depth = 0;
            else
                flw.directory_depth = 0;
        }
...
char **FS_ListFilteredFiles(
    const char *path, const char *extension, const char *filter, int *numfiles_out, qboolean allowNonPureFilesOnDisk)
{
