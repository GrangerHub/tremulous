...
static void fsc_load_file(int source_dir_id, void *os_path, char *qpath_with_mod_dir, unsigned int os_timestamp,
    unsigned int filesize, fsc_filesystem_t *fs, fsc_errorhandler_t *eh)
{
    ...
    // Get mod directory
    if (!fsc_get_leading_directory(qpath_with_mod_dir, qp_mod, sizeof(qp_mod), &qpath_start))
        return;
    if (!qpath_start)
        return;
    ...
    while ((file_ptr = fsc_hashtable_next(&hti)))
    {
        ...
        if (file->f.sourcetype != FSC_SOURCETYPE_DIRECT)
            continue;
        if (file->f.qp_name_ptr != qp_name_ptr)
            continue;
        if (file->f.qp_dir_ptr != qp_dir_ptr)
            continue;
        if (file->f.qp_ext_ptr != qp_ext_ptr)
            continue;
        if (fsc_get_mod_dir((fsc_file_t *)file, fs) != STACKPTR(qp_mod_ptr))
            continue;
        if (file->os_path_ptr && fsc_compare_os_path(STACKPTR(file->os_path_ptr), os_path))
            continue;
        if (file->pk3dir_ptr && (!pk3dir_active || fsc_strcmp((const char *)STACKPTR(file->pk3dir_ptr), pk3dir_buffer)))
            continue;
        if (!file->pk3dir_ptr && pk3dir_active)
            continue;
    ...
    else
    {
        ...
        if (pk3dir_active)
            file->pk3dir_ptr =
                fsc_string_repository_getstring(pk3dir_buffer, 1, &fs->string_repository, &fs->general_stack);
        file->f.filesize = filesize;
        file->os_timestamp = os_timestamp;
        file->refresh_count = fs->refresh_count;
    ...
    if (filename_contents & FSC_CONTENTS_DLPK3)
