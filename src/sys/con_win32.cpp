...
{
    Q_strncpyz(
        qconsole_history[qconsole_history_oldest], qconsole_line, sizeof(qconsole_history[qconsole_history_oldest]));
...
static void CON_Show(void)
{
    ...
    {
        WriteConsoleOutput(
            qconsole_hout, line + (qconsole_linelen - binfo.srWindow.Right), writeSize, writePos, &writeArea);
    }
    else
    {
...
char *CON_Input(void)
{
    ...
    // Process command like TTY console does
    if (qconsole_line[0] == '/' || qconsole_line[0] == '\\')
    {
        // Strip the leading slash/backslash and execute as command
        Q_strncpyz(text, qconsole_line + 1, sizeof(text));
    }
    else if (qconsole_line[0])
    {
        // No leading slash, treat as chat message
        Com_sprintf(text, sizeof(text), "cmd say %s", qconsole_line);
    }
    else
    {
        text[0] = '\0';
            ...
            if (*msg == '\n')
            {
                ...
                if (qconsole_hout != INVALID_HANDLE_VALUE)
                    SetConsoleTextAttribute(
                        qconsole_hout, CON_ColorVecToAttrib(g_color_table[ColorIndex(COLOR_WHITE)]));
                fputs("\n", stderr);
                msg++;
            }
            else
