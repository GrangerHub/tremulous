...
FILE *Sys_Mkfifo(const char *ospath)
{
    ...
    {
        // If it's already a pipe, just open it
        if (S_ISFIFO(buf.st_mode))
        {
            // Use open() with O_NONBLOCK to prevent blocking on empty pipe
            fd = open(ospath, O_RDWR | O_NONBLOCK);
            if (fd == -1)
            {
                Com_Printf(S_COLOR_YELLOW
                    "WARNING: Could not open existing pipe %s. "
        ...
        // If it's a regular file or other type, don't overwrite it
        else
        {
            Com_Printf(S_COLOR_YELLOW
                "WARNING: File %s already exists and is not a pipe. "
                "com_pipefile will not be used to avoid overwriting existing data.\n",
                ospath);
            return NULL;
        }
    }

    // No file exists, create new pipe
    result = mkfifo(ospath, 0600);
    if (result != 0)
        return NULL;

    // Open with O_NONBLOCK to prevent blocking when pipe is empty
    fd = open(ospath, O_RDWR | O_NONBLOCK);
    if (fd == -1)
    {
        Com_Printf(S_COLOR_YELLOW
            "WARNING: Could not open newly created pipe %s. "
            "com_pipefile will not be used.\n",
            ospath);
        // Remove the pipe we just created since we can't use it
        unlink(ospath);
        return NULL;
    }
    ...
    {
        Com_Printf(S_COLOR_YELLOW
            "WARNING: Could not get FILE* for pipe %s. "
            "com_pipefile will not be used.\n",
            ospath);
        close(fd);
        unlink(ospath);
