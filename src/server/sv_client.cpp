...
static bool SV_RSA_VerifySignature(const char *pubkey, const char *signature, const char *data, char *fingerprint)
{
    ...
    // Reinitialize the hash for the fingerprint calculation
    sha256_init(&sha256_hash);
...
void SV_GetChallenge(netadr_t from)
{
    ...
    if (sv_protect->integer & SVP_IOQ3)
    {
        if (SVC_RateLimitAddress(from, 10, 1000))
        {
            ...
            // );
            SV_WriteAttackLog(
                va("SV_GetChallenge: rate limit from %s exceeded, dropping request\n", NET_AdrToString(from)));
            return;
        }
    }
...
void SV_DirectConnect(netadr_t from)
{
    ...
    if (sv_protect->integer & SVP_IOQ3)
    {
        ...
        {
            SV_WriteAttackLog(
                va("Bad direct connect - rate limit from %s exceeded, dropping request\n", NET_AdrToString(from)));
            return;
        }
    }
...
void SV_UserinfoChanged(client_t *cl)
{
    ...
    // color codes
    Q_ApproxStrHexColors(cl->name, cl->name_ansi, sizeof(cl->name), sizeof(cl->name_ansi));
