...
static void GLimp_DetectAvailableModes(void)
{
    ...
    if (desktopMode == NULL)
    {
        ri.Printf(PRINT_WARNING, "Couldn't get desktop display mode, no resolutions detected: %s\n", SDL_GetError());
        return;
    }

    SDL_DisplayMode **modeList = SDL_GetFullscreenDisplayModes(display, &numSDLModes);
    if (modeList == NULL || numSDLModes <= 0)
    {
...
static int GLimp_SetMode(bool failSafe, bool fullscreen, bool noborder, bool coreContext)
{
...
#ifdef USE_ICON
    icon = SDL_CreateSurfaceFrom(CLIENT_WINDOW_ICON.width, CLIENT_WINDOW_ICON.height, SDL_PIXELFORMAT_RGBA32,
        (void *)CLIENT_WINDOW_ICON.pixel_data, CLIENT_WINDOW_ICON.bytes_per_pixel * CLIENT_WINDOW_ICON.width);
#endif
    ...
    const SDL_DisplayMode *desktopMode_p = (display != 0) ? SDL_GetDesktopDisplayMode(display) : NULL;
    if (desktopMode_p != NULL)
    {
        desktopMode = *desktopMode_p;
    ...
    else if (glConfig.vidWidth != R_FAILSAFE_WIDTH && glConfig.vidHeight != R_FAILSAFE_HEIGHT)
    {
        ri.Printf(PRINT_ALL, "Setting mode %dx%d failed, falling back on mode %dx%d\n", glConfig.vidWidth,
            glConfig.vidHeight, R_FAILSAFE_WIDTH, R_FAILSAFE_HEIGHT);
    ...
    for (i = 0; i < 16; i++)
    {
       ...
       // If not allowing software GL, demand accelerated
        ...
        if ((SDL_window = SDL_CreateWindow(CLIENT_WINDOW_TITLE, glConfig.vidWidth, glConfig.vidHeight, flags)) == NULL)
        {
        ...
        {
            SDL_DisplayMode mode;
            Com_Memset(&mode, 0, sizeof(SDL_DisplayMode));
        ...
        // SDL3: SDL_GL_SetSwapInterval returns bool (true=success), not int
    ...
    if (GLimp_HaveExtension("GL_ARB_multitexture"))
    {
        if (r_ext_multitexture->value)
        {
            ...
            qglClientActiveTextureARB =
                (decltype(qglClientActiveTextureARB))SDL_GL_GetProcAddress("glClientActiveTextureARB");
...
success:
    ...
    // SDL3 removed hardware gamma (SDL_SetWindowBrightness/SDL_SetWindowGammaRamp).
    // Shader-based gamma post-processing will handle brightness/gamma correction.
    ...
    if (qglGetString(GL_EXTENSIONS))
        ...
        Q_strncpyz(
            glConfig.extensions_string, "Not available (core context, fixme)", sizeof(glConfig.extensions_string));
