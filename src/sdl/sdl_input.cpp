...
// SDL3 compatibility: SDL_Keysym was removed in SDL3, fields are now in SDL_KeyboardEvent directly
typedef struct {
...
// SDL3 compatibility: KMOD_RESERVED was removed in SDL3
#ifndef KMOD_RESERVED
...
===============
*/
static void IN_PrintKey(const SDL_Keysym_compat *keysym, keyNum_t key, bool down)
{
    if (down)
        Com_Printf("+ ");
    else
...
===============
*/
static keyNum_t IN_TranslateSDLToQ3Key(SDL_Keysym_compat *keysym, bool down)
{
    ...
    else if (keysym->sym >= SDLK_SPACE && keysym->sym < SDLK_DELETE)
    {
        ...
        key = (keyNum_t)keysym->sym;
                 ...
                 ((SDL_GetModState() & SDL_KMOD_NUM) == (SDL_KMOD_NUM))))
    {
...
static void IN_GobbleMotionEvents(void)
{
    ...
    SDL_PumpEvents();
    while ((val = SDL_PeepEvents(dummy, 1, SDL_GETEVENT, SDL_EVENT_MOUSE_MOTION, SDL_EVENT_MOUSE_MOTION)) > 0)
    {
    }
...
static int hat_keys[16] = {K_JOY29, K_JOY30, K_JOY31, K_JOY32, K_JOY25, K_JOY26, K_JOY27, K_JOY28, K_JOY21, K_JOY22,
    K_JOY23, K_JOY24, K_JOY17, K_JOY18, K_JOY19, K_JOY20};
...
static void IN_InitJoystick(void)
{
    ...
    // SDL_INIT_GAMEPAD for SDL_OpenJoystick() to work correctly,
    ...
    if (!SDL_WasInit(SDL_INIT_JOYSTICK))
    {
        ...
        // SDL3: SDL_Init returns bool (true=success), not int(0=success)
    ...
    // SDL3: Joystick enumeration now uses SDL_JoystickID instead of device index
    ...
    SDL_JoystickID selectedID = joystickIDs[in_joystickNo->integer];
    SDL_free(joystickIDs);

    stick = SDL_OpenJoystick(selectedID);
    ...
    if (SDL_IsGamepad(selectedID))
        gamepad = SDL_OpenGamepad(selectedID);

    Com_DPrintf("Joystick %d opened\n", in_joystickNo->integer);
    Com_DPrintf("Name:       %s\n", SDL_GetJoystickNameForID(selectedID));
    Com_DPrintf("Axes:       %d\n", SDL_GetNumJoystickAxes(stick));
    Com_DPrintf("Hats:       %d\n", SDL_GetNumJoystickHats(stick));
    Com_DPrintf("Buttons:    %d\n", SDL_GetNumJoystickButtons(stick));
    // SDL3 removed joystick balls support
    Com_DPrintf("Use Analog: %s\n", in_joystickUseAnalog->integer ? "Yes" : "No");
    Com_DPrintf("Is gamepad: %s\n", gamepad ? "Yes" : "No");

    // SDL3: Enable joystick and gamepad events
...
static void IN_GamepadMove(void)
{
    ...
    for (i = 0; i < SDL_GAMEPAD_AXIS_COUNT; i++)
    {
        ...
        if (axis != oldAxis)
        {
            const int negMap[SDL_GAMEPAD_AXIS_COUNT] = {
                K_PAD0_LEFTSTICK_LEFT, K_PAD0_LEFTSTICK_UP, K_PAD0_RIGHTSTICK_LEFT, K_PAD0_RIGHTSTICK_UP, 0, 0};
            const int posMap[SDL_GAMEPAD_AXIS_COUNT] = {K_PAD0_LEFTSTICK_RIGHT, K_PAD0_LEFTSTICK_DOWN,
                K_PAD0_RIGHTSTICK_RIGHT, K_PAD0_RIGHTSTICK_DOWN, K_PAD0_LEFTTRIGGER, K_PAD0_RIGHTTRIGGER};
...
static void IN_JoyMove(void)
{
    ...
    // SDL3 removed joystick ball support (SDL_GetNumJoystickBalls/SDL_GetJoystickBall)
    // Ball input was rarely used; if needed in future, implement via touchpad/trackball APIs
    ...
    while (SDL_PollEvent(&e))
    {
        switch (e.type)
        {
            case SDL_EVENT_KEY_DOWN:
                ...
                // SDL3: Build compat keysym from flattened KeyboardEvent fields
                {
                    SDL_Keysym_compat keysym = {e.key.scancode, e.key.key, e.key.mod};
                    if ((key = IN_TranslateSDLToQ3Key(&keysym, true)))
            ...
            case SDL_EVENT_KEY_UP:
                ...
                {
                    SDL_Keysym_compat keysym = {e.key.scancode, e.key.key, e.key.mod};
                    if ((key = IN_TranslateSDLToQ3Key(&keysym, false)))
            ...
            case SDL_EVENT_MOUSE_BUTTON_UP: {
                ...
                Com_QueueEvent(
                    in_eventTime, SE_KEY, b, (e.type == SDL_EVENT_MOUSE_BUTTON_DOWN ? true : false), 0, NULL);
            }
            break;
            ...
            // SDL3: Window events are now individual top-level events
            // instead of sub-events of SDL_WINDOWEVENT
...
void IN_Frame(void)
{
    ...
    if (!mouseActive && cls.uiInterface != 2)
    {
        ...
        IN_SetUIMousePosition((int)fx, (int)fy);
    }
